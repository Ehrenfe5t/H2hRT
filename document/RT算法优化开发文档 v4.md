...
**当前状态：方案已冻结，优先级 P2，C1~C4 完成后再进入实现。**
...
├─ C4  场景升级（SAH/坐标变换）    ── P1 模块2
│
├─ C5  全中文规范化               ── P1 全模块（贯穿C1~C4）
├─ C6  三级验证体系               ── P1 test
│
├─ C7  SBR绕射+漫散射             ── P2 模块4/5
...
v4 完成后，开题报告中的 RT 算法需求将**全部满足或被明确标注为延后项**（时变/多普勒/实测对照 → v5）。论文"算法验证"章节所需的所有证据（L1解析解+ L2交叉验证 + L3统计验证）均有自动化生成能力。

### 11.4 v4 文档最终说明

本文档（`RT算法优化开发文档 v4.md`）是第四轮开发的完整方案。C0~C9 共 10 个批次，覆盖 EM 精度、SBR 规模、场景升级、验证体系、中文化。
