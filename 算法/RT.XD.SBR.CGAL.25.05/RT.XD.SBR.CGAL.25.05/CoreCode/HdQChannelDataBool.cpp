...
#include"HdQChannelDataBool.h"
#include"DxQStructureToString.h"

namespace ChannelDataBoolStd {


	ChannelDataBool::ChannelDataBool()
	{
		this->id_tx = -1;
		this->id_rx = -1;
		this->value = false;
	...
	std::string ChannelDataBool_To_string(const std::vector<ChannelDataBoolStd::ChannelDataBool>& bigChannelParameters) {
        ...
        for (int i = 0; i < (int)bigChannelParameters.size() - 1; i++) {
            ...
            context.append(StructureToStringStd::BoolToStringByStringStream(bigChannelParameters[i].value)); context.append(",");
        ...
        if (bigChannelParameters.size() > 0) {
            ...
            context.append(StructureToStringStd::IntToStringByStringStream(bigChannelParameters[i].id_rx)); context.append(",");
            context.append(Point3DToString(bigChannelParameters[i].location_rx, ",", "", 16)); context.append(",");
            context.append(StructureToStringStd::BoolToStringByStringStream(bigChannelParameters[i].value));
        }
